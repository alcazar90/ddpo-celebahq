...
from trayectory_metrics import sample_denoised_data_from_celebahq_intermediate_step
from trayectory_metrics import sample_denoised_data_from_celebahq
# Set up logging----------------------------------------------------------------
...
for seed in eval_seeds:
    ...
    for step in initial_steps:
        data_intermediate = sample_denoised_data_from_celebahq_intermediate_step(
            num_samples, scheduler, image_pipe, device, step, data["trajectory"][-1][0], random_seed=rnd_seed
        )
        logging.info("Computing rewards")
        rewards = []
        for xt in data_intermediate["trajectory"]:
            rewards.append(reward_fn(xt.to(device)).cpu())
