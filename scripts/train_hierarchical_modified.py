...
beta_optimal_reward = args.beta_optimal_reward
...
config = {
    ...
    "beta_optimal_reward": beta_optimal_reward,
}
...
for epoch in master_bar(range(num_epochs)):
    ...
    for _ in progress_bar(range(num_batches//num_batches)):
        initial_batch_all_step_preds, initial_batch_log_probs, initial_initiate_train_step = improved_sample_from_ddpm_celebahq(
            num_batches,
            scheduler,
            image_pipe,
            device,
    ...
    # so that the optimal window is chosen for each batch of images
    # Determine the number of initial images
    num_initial_images = len(initial_batch_all_step_preds[-1])
    # List to store optimal steps for each image
    optimal_steps = []
    ...
    for i in progress_bar(range(num_batches)):
        ...
        # Ensure the initial image is a torch tensor on the correct device
        if not isinstance(initial_image, torch.Tensor):
            initial_image = torch.tensor(initial_image).to('cuda')
        else:
            initial_image = initial_image.to('cuda')

        logging.info(f"Processing initial image {i+1}/{num_initial_images}")

        # Create an instance of the class
        finder = OptimalIntermediateStepFinder(
        ...
        optimal_step = finder.run()
        optimal_steps.append(optimal_step)

        logging.info(f"Optimal intermediate step for image {i+1}: {optimal_step}")

    # After processing, you have a list of optimal steps corresponding to each initial image
    print("Optimal steps for each initial image:", optimal_steps)
        
    for i in progress_bar(range(num_batches)):
        # sample from the model given the initial states
        batch_all_step_preds, batch_log_probs, initiate_train_step = improved_sample_from_ddpm_celebahq_initial_step(
            batch_size,
            scheduler,
            image_pipe,
            device,
