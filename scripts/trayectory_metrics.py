...
############################################################################################################
# funciones de sampling solo immagenes finales, con sus respectivos rewards
@torch.no_grad()
def sample_denoised_images_from_celebahq(
    num_samples,
    scheduler,
    image_pipe,
    device,
    ...
    for i, t in tqdm(enumerate(scheduler.timesteps)):
        ...
        if t == scheduler.timesteps[-1]:
            final_images.append(x_0_hat.clone().detach().cpu())
        # trajectory.append(xt.clone().detach().cpu())
        # trajectory_denoised.append(x_0_hat.clone().detach().cpu())
        # if t != scheduler.timesteps[-1]:
        #     trajectory.append(xt.clone().detach().cpu())
